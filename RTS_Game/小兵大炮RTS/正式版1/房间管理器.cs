...
public class Django_RoomManager : MonoBehaviourPunCallbacks
{
    ...
    void resh()
    {
        ...
        foreach (GameObject i in playerui_list)
        {
            if (i.GetComponent<PhotonView>().IsMine == true)
            {
                ...
                if (i.GetComponent<PlayerUI>().did_ready == true)
                {
    ...
    public void pre_join_set(int number)
    {
        if (player_uii.GetComponent<PlayerUI>().did_ready == false)
        {
            ...
            //根据数字切换identity
            //点击座位的时候就切换身份，而不是准备时候确认
            if (number == 1)
            {
                GameObject.Find("Identity").GetComponent<Identity>().my_identity = Identity.identity_.red;
            }
            else if (number == 2)
            {
                GameObject.Find("Identity").GetComponent<Identity>().my_identity = Identity.identity_.yellow;
            }
            else if (number == 3)
            {
                GameObject.Find("Identity").GetComponent<Identity>().my_identity = Identity.identity_.blue;

            }
            else if (number == 4)
            {
                GameObject.Find("Identity").GetComponent<Identity>().my_identity = Identity.identity_.green;
