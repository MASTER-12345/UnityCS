...
public class Django_EyesControler : MonoBehaviour //Move_Scal是脚本的名字，要改成一致的
{
    ...
    void Start()
    {
        ...
        //####################
        //将gameunits下子物体找出来，加入register列表
        StartCoroutine(Late_Register());


    }

    IEnumerator Late_Register()
    {
        yield return new WaitForSeconds(1f);
  
        for (int i = 0; i < Game_units.transform.childCount; i++)
        {
            //Debug.LogError(Game_units.transform.GetChild(i).name);
            register_List.Add(Game_units.transform.GetChild(i).gameObject);
        }
        StopCoroutine(Late_Register());
    }
